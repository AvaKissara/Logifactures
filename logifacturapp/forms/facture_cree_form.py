...
class FactureForm(forms.ModelForm):
    ...
    class Meta:
        model = Facture
        fields = ['num_facture', 'date_facture', 'total_ht_facture', 'tva_facture', 'total_ttc_facture', 'cat_facture', 'fournisseur', 'client', 'devise', 'methode_paiement','statut_facture']
        labels = {
    ...
    def __init__(self, *args, **kwargs):
        ...
        for field_name, field in self.fields.items():
            # Si c'est un champ de type choix (select)
            if isinstance(field.widget, forms.Select):
                # Utiliser le label comme valeur par défaut
                field.empty_label = field.label
            # Si c'est un champ de type input (text, number, etc.)
            else:
                # Utiliser le label comme placeholder
